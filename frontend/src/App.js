...
import ResumePdf from "./assets/Aditya_Shukla_Resume.pdf";
import "./App.css";
...
function App({
  ...
  const sendQuery = useCallback(
    async (userQuery) => {
      ...
      // 1) get optimized query (optional — fall back to raw query on failure)
      ...
      try {
        ...
        optimized = optRes.optimizedQuery || trimmed;
      } catch {
        /* rate-limited or unavailable — use raw query */
      }
      ...
      try {
        ...
        // 8) Snapshot memory (fire-and-forget — failure is non-fatal)
        let newMem = conversationMemory;
        try {
          ...
          if (!cancelRef.current) newMem = memRes.data.memory || conversationMemory;
        } catch {
          /* memory update failed — keep previous memory */
        }
        if (cancelRef.current) throw new Error("cancelled");
  ...
  useEffect(() => {
    ...
    if (initialUrlAction.triggerResumeDownload) {
      const t = setTimeout(() => {
        ...
        a.href = ResumePdf;
        a.download = "Aditya-Shukla-Resume.pdf";
        ...
        {!isWindowModalVisible && (
          <div className="ai-chat-container">
            ...
            <motion.div
              ...
              onClick={() => {
                addTab("AIChatTab", { title: "Aditya's AI Companion" });
              }}
              title="Links"
