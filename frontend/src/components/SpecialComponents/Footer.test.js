...
describe("Footer", () => {
  ...
  it("renders Aditya Shukla heading", () => {
    render(<Footer isBatterySavingOn={false} addTab={mockAddTab} />);
    expect(screen.getByRole("heading", { name: /Aditya Shukla/i })).toBeInTheDocument();
  });

  it("renders tagline Full Stack Developer & Competitive Programmer", () => {
    render(<Footer isBatterySavingOn={false} addTab={mockAddTab} />);
    expect(screen.getByText(/Full Stack Developer & Competitive Programmer/i)).toBeInTheDocument();
  });
  ...
  it("renders copyright with current year", () => {
    ...
    expect(screen.getByText(new RegExp(`© ${year} Aditya Shukla`))).toBeInTheDocument();
  });
});
