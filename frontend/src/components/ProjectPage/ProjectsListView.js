    ...
    <>
      ...
      <div ref={parentRef} className="project-container">
        {projects.map((project, index) => {
          ...
          // Mount content for all cards when total count is small (≤10). For larger lists, use a sliding window around the visible card.
          const hasContentMounted =
            projects.length <= 10
              ? true
              : visibleCardIndex === null
                ? index < 6
                : index >= Math.max(0, visibleCardIndex - 3) &&
                  index <= visibleCardIndex + 3;
