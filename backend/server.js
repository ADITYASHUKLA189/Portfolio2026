...
// Global endpoints
app.get("/", (req, reply) =>
  reply.send("Welcome to Aditya's MERN Portfolio Backend")
);
app.get("/api", (req, reply) =>
  reply.send("This is the API track for Aditya's MERN Portfolio Backend.")
);
...
(async () => {
  try {
    // Register routes first (they lazy-access DB via getDB())
    ...
    // Bind port first so Render detects the service is alive
    const PORT = process.env.PORT || 5000;
    await app.listen({ port: PORT, host: "0.0.0.0" });
    console.log(`🚀 Server listening on port ${PORT}`);

    // Then connect to MongoDB and initialize AI context
    await connectDB();
