...
/**
 * Retry wrapper for OpenAI/Gemini calls that may return 429 rate-limit errors.
 * Waits an increasing delay between each attempt.
 */
async function callLLMWithRetry(fn, retries = 2, baseDelayMs = 30000) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (is429 && attempt < retries) {
        ...
        console.warn(`⚠️  Gemini 429 rate limit hit. Retrying in ${delay / 1000}s… (attempt ${attempt}/${retries})`);
...
async function fetchAllRepos() {
  if (!GITHUB_TOKEN) {
    console.warn("⚠️ GITHUB_TOKEN not set — skipping GitHub context");
    return [];
  }
  let repos = [],
    page = 1;
...
async function getEmbedding(text) {
  ...
  if (!data.embedding) throw new Error("Gemini embedding failed: " + JSON.stringify(data));
...
async function optimizeQuery(conversationMemory, userQuery) {
  ...
  const systemPrompt = `
You are Aditya Shukla's (He/Him/His) expert query optimizer for his AI ChatBot, responsible for rewriting user queries to guarantee precise hits across his indexed knowledge base (experiences, honors experiencs, involvements, projects, skills, honors year in reviews, resume data, and github repositories).
...
`.trim();
  ...
  // Dynamically determine max tokens — must be large enough for thinking model overhead (~800 tokens)
  let maxTokens = Math.ceil((userQuery.length / 4) * 2) + 1000;
  if (maxTokens < 1500) maxTokens = 1500;
    ...
    messages: [
      {
        role: "system",
        content: `
        You are Aditya Shukla (He/Him/His), a 1st-year B.Tech IT student at the IIIT-Bhubaneswar.  Speak always in first person as Aditya (He/Him/His), and never as “the assistant” or “the bot.”  Keep paragraphs short (2-4 sentences), narrative, friendly-expert in tone, and never use slang or emojis.
        Answer only about Aditya and strictly based on context in English. Do not invent examples or data. NEVER GO OUT OF TOPIC!!! keep this conversational and in a easy to understand language 

        **Core Rules**  
        1. **First-Person Only**: Always answer as “I” (Aditya Shukla). Do not repeat your name in every sentence—your voice is implicit.  
...
async function suggestFollowUpQuestions(query, answer, conversationMemory) {
  ...
  const systemContent = `
  You are an assistant bot for the master AI ChatBot of Aditya Shukla (He/Him/His), a 1st-year B.Tech IT student at IIIT-Bhubaneswar. You're a highly respected helper whose sole job is to suggest concise, intelligent follow-up questions that continue a user's conversation about Aditya—nothing else. Always keep your suggestions grounded in what's known from his materials, in English, and never go off-topic.
  ...
  **Core Rules**  
  ...
  3. **Strict Relevance**: Each question must build on the user's last query and the AI's answer, focusing solely on Aditya's experiences or expertise.  
  ...
  **Stylistic Guidelines**  
  ...
  - **Terminology Consistency**: Use terms from Aditya's profile (e.g., “project,” “internship,” “dashboard”).
  ...
  **Closing Reminder**  
  If context is insufficient, do not guess—simply indicate you need more details. Always maintain a friendly-expert tone and stay focused on Aditya's documented experiences.
  `.trim();
  ...
  // Dynamic max_tokens — must be large enough for thinking model overhead (~800 tokens)
  let maxTokens = Math.ceil((query.length / 4) * 4.5) + 1000;
  if (maxTokens < 1500) maxTokens = 1500;
...
async function snapshotMemoryUpdate(previousMemory, query, answer, messageid) {
  ...
  **Stylistic Guidelines**  
  ...
  const prevMemoryTokens = previousMemory
    ...
    : 0;
  let maxTokens = prevMemoryTokens + 1000;
  if (maxTokens < 1500) maxTokens = 1500;
