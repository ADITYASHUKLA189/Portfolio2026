...
async function updateDynamicImagesCache(retryInterval = 1000) {
  ...
  try {
    ...
    // DB not connected yet — retry later
  }
  if (!db) {
...
const compareAdminPassword = async (request, reply) => {
  ...
  try {
    ...
    await db.collection("AdityaPortfolioOTP").deleteMany({});
    await db.collection("AdityaPortfolioOTP").insertOne({ otp, expireTime });
    reply.send({
      success: true,
      otpSent: true,
      otp: otp,
...
const compareOTP = async (request, reply) => {
  ...
  const db = getDB();
  try {
    const otpData = await db.collection("AdityaPortfolioOTP").findOne({});
    ...
    await db.collection("AdityaPortfolioOTP").deleteOne({});
  ...
  try {
    ...
    const collections = {
      ...
      yearInReviewTable: await db
        .collection("yearInReviewTable")
        .countDocuments({ deleted: { $ne: true } }),
      AdityaPortfolio: await db
        .collection("AdityaPortfolio")
        .countDocuments(),
      FeedTable: await db
        .collection("FeedTable")
        .countDocuments({ deleted: { $ne: true } }),
